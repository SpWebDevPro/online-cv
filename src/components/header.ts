...
import cvFile from "../assets/CV_Sandrine_Pradier_2.pdf";
...
export class MyHeader extends LitElement {
  ...
  render() {
    ...
    return html`
      <div>
        <img src=${this.imageurl} alt="photo ${
      this.resume.basics.name
    }" class="avatar ${this.imagedisplay}">
        ...
        <div class="title-bar header-title">
          ...
          <div class="icon-links displayflex">
            <ul class=liststylenone>
              <li class="icon-link-item"><a href="tel:${
                this.contactphone
              }" ><img src=${phone} alt="${this.contactphone}" width="${
      this.width
    }" height="${this.width}" /></a></li>
              <li class="icon-link-item"><a href="mailto:${
                this.contactemail
              }" ><img src=${email} alt="${this.contactemail}" width="${
      this.width
    }" height="${this.width}" /></a></li>
            </ul>
            <ul class="liststylenone listsocialicon">
              ${map(
                this.socialprofiles,
                (sp) =>
                  html`<li class="icon-link-item">
                    <a href="${sp.url}" class="icon-link"
                      ><img
                        src=${sp.image}
                        alt="${sp.network} logo"
                        width="${this.width}"
            ...
            <ul class=liststylenone>
              <li class="icon-link-item"><a href="${
                this.contactwebsite
              }" ><img src=${website} alt="${this.contactwebsite}" width="${
      this.width
    }" height="${this.width}" /></a></li>
            </ul>
          </div>
